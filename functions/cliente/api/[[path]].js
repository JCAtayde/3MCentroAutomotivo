...
export async function onRequest(context){
  ...
  try{
    ...
    if(rota==='ativar' && request.method==='POST'){
      ...
      const cli = await env.DB.prepare('SELECT id, nome, cpf, telefone1, zap_num FROM clientes WHERE id=?').bind(cv.cliente_id).first();
      if(!cli) return json({ error:'Cliente não encontrado.' }, 404);
      if(dig(b.cpf).length!==11 || dig(b.cpf)!==dig(cli.cpf)) return json({ error:'CPF não confere com o nosso cadastro.' }, 403);
      const zapCad = dig(cli.zap_num||cli.telefone1); const zapInf = dig(b.zap);
      if(zapInf.length<8 || zapCad.slice(-8)!==zapInf.slice(-8)) return json({ error:'O WhatsApp não confere com o nosso cadastro.' }, 403);
      ...
      if(ja) await env.DB.prepare('UPDATE portal_contas SET cpf=?, zap=?, senha_hash=?, salt=?, ativo=1, atualizado=? WHERE id=?').bind(dig(cli.cpf), zapCad, senha_hash, salt, Date.now(), ja.id).run();
      else await env.DB.prepare('INSERT INTO portal_contas (id,cliente_id,cpf,zap,senha_hash,salt,ativo,criado,atualizado) VALUES (?,?,?,?,?,?,1,?,?)').bind(randomHex(12), cli.id, dig(cli.cpf), zapCad, senha_hash, salt, Date.now(), Date.now()).run();
